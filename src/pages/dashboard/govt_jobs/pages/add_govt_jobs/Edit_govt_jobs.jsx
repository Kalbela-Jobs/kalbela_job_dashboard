...
import { Button, DatePicker, Divider, Form, Input, InputNumber, Upload, Select, Modal, message } from "antd"
...
const EditGovtJobs = ({ data, onClose, base_url }) => {
      ...
      const [publicationDate, setPublicationDate] = useState(data?.publicationDate ? moment(data.publicationDate) : null)
      ...
      useEffect(() => {
            form.setFieldsValue({
                  ...data,
                  applicationStartDate: data?.applicationStartDate ? moment(data.applicationStartDate).local() : null,
                  applicationDeadline: data?.applicationDeadline ? moment(data.applicationDeadline).local() : null,
                  publicationDate: data?.publicationDate ? moment(data.publicationDate).local() : null,
      ...
      const handleEditJob = async () => {
            ...
            const value = {
                  ...
                  uploadDocument: input_value.document_url ? input_value.document_url : data.uploadDocument, // Attach the document URL
            }
            console.log(value, "data_update_value")
            try {
                  const response = await fetch(`${base_url}/jobs/update-govt-job?job_id=${data._id}`, {
                        method: "PUT",
      ...
      const selectedOrg = organizations.find((org) => org._id === job?.organization?.id)
      ...
      return (
            <>
                  <style>{styles}</style>
                  <Modal title="Edit Govt Jobs" onCancel={() => onClose()} open={data} footer="" width={800}>
                        <Form form={form} layout="vertical" onFinish={handleEditJob}>
                              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                    ...
                                    >
                                          <Select placeholder="Select an organization" showSearch optionFilterProp="children">
                                                {organizations.map((org) => (
                                                      <Option
                                                            key={org._id}
                                                            value={JSON.stringify({ id: org._id, name: org.name, logo: org.logo, description: org.description, website: org?.org_website })}
                                                      >
                                    ...
                                    <Form.Item
                                          ...
                                          rules={[{ required: false, message: "Please enter publication date" }]}
                                    >
                              ...
                              {fileList.length > 0 && fileList[0].url && (
                                    <div className="mt-4">
                                          <h4 className="text-sm font-medium mb-2">Current Document Preview:</h4>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <Form.Item name="vacancy" label="Vacancy" rules={[{ required: true, message: "Please enter vacancy" }]}>
                                          <InputNumber min={1} className="w-full" placeholder="Enter vacancy" />
                                    </Form.Item>
                              </div>
                              <div className="flex justify-end gap-2 mt-4">
                                    ...
                                    <Button onClick={form.submit} className="bg-blue-500 text-white px-4 py-2 rounded">
                                          Save
                                    </Button>
                              </div>
                        </Form>
